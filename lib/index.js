...
function compileSass(options) {
    ...
    function fileChanged(path) {
        if (Array.isArray(sassFileMap[path])) {
            ...
            sassFileMap[path].forEach(function (mainFile) {
                ...
                fs.utimesSync(mainFile, new Date(), new Date());
            });
        } else {
    ...
    function watchImports(main, imports) {
        ...
        try {
          ...
          fileWatcher.add(importsToWatch);
    ...
    function middleware(req, res, next) {
        ...
        if (/\.(?:scss|sass)$/.test(req.path)) {
            ...
            if (req.fresh) {
                ...
                res.setHeader('Content-Length', Buffer.from(cache[fileUrl]).length);
                ...
                sass.render({
                    ...
                    res.setHeader('Content-Length', Buffer.from(css).length);
                    res.end(css);
